...
type FakeBucketAPI struct {
	...
	CreateHook func(ctx context.Context, in CreateInput) error
...
func (f *FakeBucketAPI) Create(ctx context.Context, in CreateInput) error {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.Creates++
	if f.CreateHook != nil {
