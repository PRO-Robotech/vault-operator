...
func main() {
	...
	if err := controller.SetupIndexers(mgr); err != nil {
	...
	targetManager := target.NewClusterManager(mgr.GetScheme())
	if err := mgr.Add(targetManager); err != nil {
		setupLog.Error(err, "unable to add target ClusterManager to manager")
		os.Exit(1)
	}

	if err := (&controller.VaultClaimReconciler{
		Client:        mgr.GetClient(),
		Scheme:        mgr.GetScheme(),
		Recorder:      mgr.GetEventRecorderFor("vaultclaim-controller"),
